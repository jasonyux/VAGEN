...
class SokobanTerminalEnv(BaseEnv):
    GRID_LOOKUP = {
        0: " # \t",  # wall
        1: " _ \t",  # floor
        2: " O \t",  # target
    ...
    def __init__(self, config:SokobanTerminalEnvConfig):
    ...
    def step(self, action_str: str):
        ...
        ### only award terminal step
        if done:
            return self._render(init_obs=False), 1.0, done, info
        else:
...
if __name__ == "__main__":
    ...
    config = SokobanTerminalEnvConfig(**kwargs)
    env = SokobanTerminalEnv(config)
